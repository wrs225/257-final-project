...
#include "obj_dir_sar_adc__N_BITS_10/Vsar_adc__N_BITS_10.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_sar_adc__N_BITS_10/Vsar_adc__N_BITS_10__Syms.h"
#include "svdpi.h"
#endif
...
extern "C" {
  ...
  #if VLINETRACE
  void trace( Vsar_adc__N_BITS_10_t *, char * );
  #endif
...
Vsar_adc__N_BITS_10_t * create_model( const char *vcd_filename ) {

  Vsar_adc__N_BITS_10_t * m;
  Vsar_adc__N_BITS_10   * model;

  Verilated::randReset( 0 );
  Verilated::randSeed( 0 );

  m     = (Vsar_adc__N_BITS_10_t *) malloc( sizeof(Vsar_adc__N_BITS_10_t) );
  model = new Vsar_adc__N_BITS_10();
  ...
  m->clk = &model->clk;
  m->eoc = &model->eoc;
  m->input_hold_digital = &model->input_hold_digital;
  m->input_voltage_real = &model->input_voltage_real;
  m->output_result_digital = &model->output_result_digital;
  m->reset = &model->reset;
...
void destroy_model( Vsar_adc__N_BITS_10_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  Vsar_adc__N_BITS_10 * model = (Vsar_adc__N_BITS_10 *) m->model;
...
void comb_eval( Vsar_adc__N_BITS_10_t * m ) {
...
void seq_eval( Vsar_adc__N_BITS_10_t * m ) {
...
#if VLINETRACE
void trace( Vsar_adc__N_BITS_10_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_sar_adc___05FN_BITS_10__v );
